...
def training_loop():
    ...
    while True :
        ...
        while not done:
            ...
            elif reward == 0 and done == True :  # if the agent falls into the hole, agent gets -1 point
                reward = -1
            
            elif now_state == new_state : # if the agent walks to the wall, agent gets -1 point
                reward = -1
                done = True
            
            elif past_state == new_state : # if the agent returned past state, agent gets -1 point
                reward = -1
                done = True
...
def test_for_save(policy_dqn):
    ...
    while not done:
        ...
        if action_counter > 15:  # if action_counter exceeds 15, agent is stuck
            env2.close()
            return reward
